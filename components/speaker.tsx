...
const SpeakerContextBridge = ({ 
  ...
  className?: string
  currentStation?: { title: string; streamUrl: string } | null
  ...
  return useMemo(
    () => <SpeakerControls 
      ...
      audioRef={audioRef}
    />,
    [className, currentStation, audioRef]
  )
}
...
function SpeakerControls({
  ...
  const currentTrack = currentStation 
    ? { title: currentStation.title.substring(0, 50), artist: "" }
    : tracks[currentTrackIndex]

  // Connect external audio ref to internal audio player when station changes
  useEffect(() => {
